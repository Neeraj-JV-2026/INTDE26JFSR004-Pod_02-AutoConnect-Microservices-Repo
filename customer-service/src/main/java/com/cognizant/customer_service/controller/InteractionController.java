...
public class InteractionController {
    ...
    @GetMapping
    @PreAuthorize("hasAnyAuthority('ADMIN', 'SALES_CONSULTANT', 'SERVICE_ADVISOR', 'AUDITOR')")
    @Operation(summary = "Get all interactions")
    public ResponseEntity<List<InteractionDTO>> getAllInteractions() {
        List<InteractionDTO> list = interactionService.getAllInteractions().stream()
                .map(this::mapToInteractionResponse).collect(Collectors.toList());
        return ResponseEntity.ok(list);
    }

    @GetMapping("/{customerId}")
    @PreAuthorize("hasAnyAuthority('ADMIN', 'SALES_CONSULTANT', 'SERVICE_ADVISOR', 'AUDITOR', 'CUSTOMER')")
